...
public class JoelibSmartsTest {
	...
	public void testDirectJoeMolMatch() throws Exception {
		...
		/*
		 * try{ String Thioamide =
		 * "[$([CX3;!R][#6]),$([CX3H;!R])](=[SX1])[#7X3;$([H2]),$([H1][#6;!$(C=[O,N,S])]),$([#7]([#6;!$(C=[O,N,S])])[#6;!$(C=[O,N,S])])]"
		 * ; //String Hydroperoxide = "[OX2H][OX2]"; //String Aldehyde =
		 * "[$([CX3H][#6]),$([CX3H2])]=[OX1]"; //String Ketene =
		 * "[CX3]=[CX2]=[OX1]"; //String Epoxide = "[OX2r3]1[#6r3][#6r3]1";
		 * //String Ketone = "[#6][CX3](=[OX1])[#6]"; //String Alkene =
		 ...
		 * //Carbimazole
		 * assertTrue(rule.verifyRule(FunctionalGroups.createAtomContainer
		 * ("CCOC(=O)N1C=CN(C)C1(=S)",true))); //Ethylene
		 * //assertTrue(rule.verifyRule
		 * (FunctionalGroups.createAtomContainer("C=C",true))); //Hydrogen
		 * peroxide
		 * //assertTrue(rule.verifyRule(FunctionalGroups.createAtomContainer
		 * ("OO",true))); //Ethanal(Aldehide)
		 * //assertTrue(rule.verifyRule(FunctionalGroups
		 * .createAtomContainer("CC=O",true))); //Methyl ketene
		 * //assertTrue(rule
		 * .verifyRule(FunctionalGroups.createAtomContainer("CC=C=O",false)));
		 * //Propylene oxide
		 * //assertTrue(rule.verifyRule(FunctionalGroups.createAtomContainer
		 * ("C1OC1C",false))); //Methyl vinyl ketone
		 * //assertTrue(rule.verifyRule
		 * (FunctionalGroups.createAtomContainer("C=CC(=O)C",false))); } catch
		 * (SMARTSException x) { fail(); } catch
		 * (toxTree.exceptions.DecisionMethodException x) { fail(); }
		 */
	}
}
