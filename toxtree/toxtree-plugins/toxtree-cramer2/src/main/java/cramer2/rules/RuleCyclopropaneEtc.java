...
/**
 * Rule 25 of the Cramer scheme (see {@link cramer2.CramerRulesExtendedExtended})
...
public class RuleCyclopropaneEtc extends RuleRingAllowedSubstituents {
	...
	public RuleCyclopropaneEtc() {
		...
		explanation.append("<LI>(a)a cyclopropane or cyclobutane with only the substituents mentioned in Q24 or");
	...
	protected IRingSet hasRingsToProcess(IAtomContainer  mol) throws DecisionMethodException {
	    ...
	    if (rings.getAtomContainerCount() > 1) return null; //monocarbocyclic
	    IRing r = (IRing) rings.getAtomContainer(0);
	    if (r.getAtomCount() > 4) return null;
	    Object o = r.getProperty(MolAnalyser.HETEROCYCLIC);
	    if ((o!=null) && ((Boolean) o).booleanValue()) return null;
		else return rings;
