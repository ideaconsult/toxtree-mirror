...
public class ToxForestDataModulePanel extends DataModulePanel {
	...
	protected void addWidgets(ActionList actions) {
		...
		table = new JTable(model,createcolumns(model)) {
		    ...
		    public TableCellRenderer getCellRenderer(int row, int column) {
                ...
                //if ((row % 2) ==0) setBackground(new Color(250,250,250)); else setBackground(Color.white);
    ...
    protected void removeMethod(DecisionResultsList methods,int row) {
        try {
            if (JOptionPane.showConfirmDialog(this, "Remove \""+ 
                    methods.getMethod(row).toString() + "\" ?"
    ...
    protected TableColumnModel createcolumns(TableModel model) {
        ...
        for (int i=0; i < model.getColumnCount();i++) {
            ...
            if (i==3) c.setCellRenderer(new CategoryTableCellRenderer());
            else c.setCellRenderer(r);
            if (i==0) c.setMaxWidth(32);
            else if (i==1) c.setMaxWidth(48);
            else c.setMinWidth(64);
            columns.addColumn(c);
