 ...
 * {@link toxTree.core.IDecisionMethod}, {@link toxTree.core.IDecisionRule},
 * and {@link toxTree.core.IDecisionCategory} in all .jar files in a user
 * defined directory.
 * 
...
public class Introspection {
	 ...
	 * Calls {@link #configureURLLoader(File directory)} for each of {@link #defaultLocation} directories.
	 * @param classLoader
	 */
	public static void configureURLLoader(ClassLoader classLoader) {
		...
		for (int i=0;i < defaultLocation.length;i++) {
			try {
				configureURLLoader(new File(defaultLocation[i]));
	...
	public static ToxTreePackageEntries implementInterface(ClassLoader classLoader,
			File directory, String interfaceName) throws toxTree.exceptions.IntrospectionException {
		setLoader(loader);
	 ...
	 * Uses {@link #getAvailableTypes(ClassLoader, String)}
	...
	/**
	 * This method is the core of {@link toxTree.apps.ToxTreeApp} extension mechanism.
	 ...
	 * <li>The directory from where the application was started (current directory "."). This is 
	 * usually the directory where toxTree.jar resides.
		...
		for (int i=0;i < defaultLocation.length;i++) {
			try {
				ToxTreePackageEntries list = implementInterface(classLoader, 
						new File(defaultLocation[i]),
						interfacename);
				if (list.size() != 0) {
					if (listAll == null) listAll = new ToxTreePackageEntries();
					listAll.addAll(list);
				}
			} catch (toxTree.exceptions.IntrospectionException x) {
				x.printStackTrace();
		...
		if ((loader == null) || !(loader instanceof ToxTreeClassLoader)) {
			...
			for (int i = 0; i < defaultURL.length; i++) {
				try {
					 ...
					 defaultURL[i] = new URL("jar", "",defaultLocation[i] + "!/");
	...
	/**
	 * Loads a decision tree from an InputStream
    ...
    public static IDecisionMethod loadRules(InputStream stream, String newTitle) throws IntrospectionException{
		try {
		    ...
		    ToxTreeObjectInputStream in = new ToxTreeObjectInputStream(stream);
    ...
    public static IDecisionMethodsList loadForest(InputStream stream) throws IntrospectionException {
		try {
		    ...
		    /*
    		JOptionPane.showMessageDialog(null,
    				methods.size() + " decision trees",
				    "Decision forest loaded successfully.",						
				    JOptionPane.INFORMATION_MESSAGE);
				    */
    		return methods;
		} catch (Exception e) {
    ...
    public static IDecisionMethodsList loadForestXML(InputStream stream) throws IntrospectionException {
		try {
			if (loader == null) setLoader(null); //this will trigger creation of the right classloader
			Thread.currentThread().setContextClassLoader(loader);
