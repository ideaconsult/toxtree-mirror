...
public class Rule21 extends RuleRingMainStrucSubstituents {
	...
	public boolean verifyRule(org.openscience.cdk.interfaces.AtomContainer  mol) throws DecisionMethodException {
		...
		nitroGroupsCount = 0;
		halogensCount = 0;		
		RingSet rs = hasRingsToProcess(mol);
	    ...
	    for (int i=0; i < rs.size(); i++) {
	        ...
	        org.openscience.cdk.interfaces.AtomContainer mc = FunctionalGroups.cloneDiscardRingAtomAndBonds(mol,r);	        
			...
			for (int k = 0; k < s.getMoleculeCount(); k++) {
				org.openscience.cdk.interfaces.Molecule m = s.getMolecule(k);
			    if (m!=null) {
	...
	public boolean substituentIsAllowed(AtomContainer a, int[] place) throws DecisionMethodException {
		if (FunctionalGroups.hasMarkedOnlyTheseGroups(a,phenolIDs)) return true;
			...
			for (int i=0;i<a.getAtomCount();i++)
				if (a.getAtomAt(i).getSymbol().equals("C")) continue;
				else if (a.getAtomAt(i).getSymbol().equals("H")) continue;
				else {
					logger.debug("Forbidden group found\t",a.getAtomAt(i).getSymbol());
					return false;
				}
