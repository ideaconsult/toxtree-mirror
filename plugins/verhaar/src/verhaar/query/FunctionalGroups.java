...
public class FunctionalGroups extends toxTree.query.FunctionalGroups {
	...
	public static QueryAtomContainer halogenAtAlphaFromUnsaturation(String[] halogens) {
		...
		for (int i=0;i<a.length;i++) {
			...
			default: {
				q.addBond(new OrderQueryBondAromatic((QueryAtom)a[i],(QueryAtom)a[i-1],CDKConstants.BONDORDER_SINGLE,false));
				break;
			}
			}
		}	
	...
	public static QueryAtomContainer halogen(String[] halogens) {
		...
		q.addAtom(h);
		q.addAtom(a);
		q.addBond(new AnyOrderQueryBond((QueryAtom)a,(QueryAtom)h,CDKConstants.BONDORDER_SINGLE));
		return q;
	...
	public static QueryAtomContainer phenol() {
		...
		for (int i=0;i<a.length;i++) {
			...
			q.addAtom(a[i]);
			if (i > 0) {
				q.addBond(new  verhaar.query.AromaticQueryBond((QueryAtom)a[i],(QueryAtom)a[i-1],CDKConstants.BONDORDER_AROMATIC));
			}
		}
		q.addBond(new verhaar.query.AromaticQueryBond((QueryAtom)a[0],(QueryAtom)a[5],CDKConstants.BONDORDER_AROMATIC));
		...
		q.addBond(new OrderQueryBond(o,(QueryAtom)a[0],CDKConstants.BONDORDER_SINGLE));
	...
	public static QueryAtomContainer benzylAlcohol() {
		...
		try {
			HueckelAromaticityDetector.detectAromaticity(q);
	...
	public static QueryAtomContainer ketone_a_b_unsaturated() {
		...
		for (int i=0; i < a.length; i++) {
			...
			case 2: { 
				a[i] = new SymbolQueryAtom(new org.openscience.cdk.Atom("C")); 
				q.addBond(new OrderQueryBond((QueryAtom)a[i],(QueryAtom)a[i-1],CDKConstants.BONDORDER_SINGLE));
				break;}			
			case 3: { a[i] = new SymbolQueryAtom(new org.openscience.cdk.Atom("O"));
					q.addBond(new OrderQueryBond((QueryAtom)a[i],(QueryAtom)a[i-1],CDKConstants.BONDORDER_DOUBLE));
					break;}
			case 4: { 
				a[i] = new SymbolSetQueryAtom();
				((SymbolSetQueryAtom)a[i]).addSymbol("C");
				((SymbolSetQueryAtom)a[i]).addSymbol("H");
				q.addBond(new OrderQueryBond((QueryAtom)a[i],(QueryAtom)a[2],CDKConstants.BONDORDER_SINGLE));
				break;}
			}
			q.addAtom(a[i]);
	...
	public static Molecule makeNitroPhenol() {
		  ...
		  mol.addBond(b6);
		  Bond b7 = new Bond(a4, a7, 1.0);
		  mol.addBond(b7);
		  Bond b8 = new Bond(a2, a8, 1.0);
		  mol.addBond(b8);
		  Bond b9 = new Bond(a8, a9, 2.0);
		  mol.addBond(b9);
		  Bond b10 = new Bond(a8, a10, 2.0);
		  mol.addBond(b10);
		  return mol;
		}
    ...
    public static QueryAtomContainer createAutoQueryContainer(AtomContainer container) {
        ...
        for (int i = 0; i < atoms.length; i++) {
        	if (atoms[i] instanceof org.openscience.cdk.interfaces.PseudoAtom)
        		queryContainer.addAtom(new AnyAtom());
        	else if (atoms[i].getSymbol().equals("X")) { //halogen
        		SymbolSetQueryAtom a = new SymbolSetQueryAtom();
        		a.addSymbol("Cl");
        		a.addSymbol("Br");
        		a.addSymbol("F");
        		a.addSymbol("I");
        	} else queryContainer.addAtom(new SymbolAndChargeQueryAtom(atoms[i]));
        }
        ...
        for (int i = 0; i < bonds.length; i++) {
            ...
            if (bonds[i].getFlag(CDKConstants.ISAROMATIC)) {
                queryContainer.addBond(new verhaar.query.AromaticQueryBond((QueryAtom) queryContainer.getAtomAt(index1),
