 ...
 */
public class DescriptorMopacShell implements IMolecularDescriptor, PropertyChangeListener {
    protected static TTLogger logger = new  TTLogger(DescriptorMopacShell.class);
	//protected static toxTree.logging.TTLogger logger = new toxTree.logging.TTLogger(DescriptorMopacShell.class);
	public static String[] defaultparams = {"PM3 NOINTER NOMM BONDS MULLIK PRECISE GNORM=0.0",Introspection.getToxTreeRoot()+"helper","MOPAC_7.1.exe",""}; //mmff94
    ...
    */
    ...
    public DescriptorMopacShell() throws ShellException {
        ...
        this(Introspection.getToxTreeRoot()+Preferences.getProperty(Preferences.MOPAC_DIR),
                Preferences.getProperty(Preferences.MOPAC_EXE));
    }
    public DescriptorMopacShell(String mopac_path, String mopac_exec) throws ShellException {
        ...
        Preferences.getPropertyChangeSupport().addPropertyChangeListener(Preferences.MOPAC_DIR, this);
        Preferences.getPropertyChangeSupport().addPropertyChangeListener(Preferences.MOPAC_EXE, this);
    }
    @Override
    protected void finalize() throws Throwable {
        Preferences.getPropertyChangeSupport().removePropertyChangeListener(Preferences.MOPAC_DIR, this);
        Preferences.getPropertyChangeSupport().removePropertyChangeListener(Preferences.MOPAC_EXE, this);
        super.finalize();
    }
    /**
    ...
    protected void runMopac(IAtomContainer atomcontainer) throws CDKException {
        ...
        if (osName.startsWith("Mac OS")) {
            ...
            try {
                ...
                executable.append(mopac_path);
                executable.append("\\");
                executable.append(mopac_executable);
                File f = new File(executable.toString());
                if (!f.exists()) throw new CDKException(executable.toString() + " not found!");
                executable.append(" "+inFile);
           ...
           try {
                ...
                StringBuffer executable = new StringBuffer();
                executable.append(mopac_path);
                executable.append("\\linux\\");
                executable.append(mopac_executable);
                File f = new File(executable.toString());
                if (!f.exists()) throw new CDKException(executable.toString() + " not found!");
                executable.append(" "+inFile);
                
                if (!f.exists()) throw new CDKException(executable + " not found!");                
                debug("<MOPAC filename=\""+executable+"\">");
                long now=System.currentTimeMillis();                
                Process p = Runtime.getRuntime().exec(executable.toString()
                        ,new String[]{},
 ...
 */
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        try {
            setMopac_path(Preferences.getProperty(Preferences.MOPAC_DIR));
            mopac_executable = Preferences.getProperty(Preferences.MOPAC_EXE);
            //System.out.println(Preferences.getProperty(Preferences.MOPAC_DIR));
        } catch (Exception x) {
            logger.error(x);
        }
