...
public abstract class OptionsPanel<T> extends JPanel {
	...
	protected void placeWidgets() {
		...
		org.jdesktop.layout.GroupLayout glayout = new org.jdesktop.layout.GroupLayout(
				this);
		setLayout(glayout);
		...
		for (int i=0; i < labelsDescriptor.length;i++) pg.add(labelsDescriptor[i]);
		pg.add(params);
	
		for (int i=0; i < labels.length;i++) pg.add(labels[i]);
	    sg.add(pg);
	    
	    pg = glayout.createParallelGroup(GroupLayout.LEADING);
	    for (int i=0; i < textDescriptor.length;i++) pg.add(textDescriptor[i]);
	    for (int i=0; i < text.length;i++) pg.add(text[i]);
	    sg.add(pg);
		...
		/*
	    setPreferredSize(new Dimension(Integer.MAX_VALUE,(labels.length+4)*32));
